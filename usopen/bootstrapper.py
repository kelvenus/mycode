...
## Define the new function called bootstrapper
def bootstrapper(dev_type, dev_ip, dev_un, dev_pw, config):
    try:   
        ...
        open_connection.enable() # this set the connection in enable mode
        ...
        open_connection.send_command_expect('write memory') # write the memory (okay if thei gets done)
        open_connection.disconnect() # close the open connection
        return true #  Everything workded! - "Return TRUE when complete"
    expect: 
